...
type PricingTablesProps = {
  className?: string;
  /**
   * Where every "start with this" lands. The homepage keeps `#contact`, which
   * scrolls to the form already on the page; a standalone page has no such
   * section and passes the contact route instead.
   */
  contactHref?: string;
};

/**
 * The package explorer: category rail, the plans in the selected category,
 * the combo offer and the pricing note.
 *
 * Split out from <Pricing> so the homepage section and the pricing route can
 * render the same tables under different openers. Everything above the rail —
 * the section wrapper and its numbered header — belongs to the caller, because
 * that is the only part the two contexts disagree about.
 */
export function PricingTables({ className, contactHref = "#contact" }: PricingTablesProps) {
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {group.plans.map((plan, index) => (
              <PlanCard key={plan.id} plan={plan} index={index} contactHref={contactHref} />
            ))}
          </motion.div>
        </AnimatePresence>
      </div>
...
function PlanCard({
  ...
  return (
    ...
    >
      {plan.recommended ? (
        <span className="absolute inset-x-0 top-0 h-px bg-signal-bright" aria-hidden />
      ) : null}
      ...
      <div className="flex flex-col gap-4">
        ...
        <ButtonLink href={contactHref} variant="ghost" size="sm" className="w-fit">
          Start with this
        </ButtonLink>
      </div>
    </motion.article>
